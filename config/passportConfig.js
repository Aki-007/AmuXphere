    ...
    async (accessToken, refreshToken, profile, done) => {
      try {
        ...
        if (userResult.rows.length === 0) {
          ...
          const googleAuthQuery = `INSERT INTO google_auth (user_id, google_id) VALUES ($1, $2)`
          await db.query(googleAuthQuery, [userId, profile.id])
        } else {
          userId = userResult.rows[0].id
          isProfileComplete = !!userResult.rows[0].role && !!userResult.rows[0].avatar_url
        }
