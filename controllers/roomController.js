...
const joinRoom = async (req, res) => {
  try {
    ...
    await db.query("INSERT INTO participants (room_id, student_id) VALUES ($1, $2)", [room.id, user.id])
...
const getUserClassrooms = async (req, res) => {
  try {
    ...
    res.status(200).json({ classrooms: result.rows })
  } catch (err) {
...
const activateRoom = async (req, res) => {
  try {
    ...
    // Activate the room
    await db.query("UPDATE rooms SET is_active = TRUE WHERE id = $1", [id])
...
const joinClassroomSession = async (req, res) => {
  try {
    ...
    await db.query(
      "INSERT INTO session_participants (room_id, student_id, avatar_url) VALUES ($1, $2, $3) ON CONFLICT DO NOTHING",
      [id, user.id, user.avatar_url],
    )
...
const deactivateRoom = async (req, res) => {
  try {
    ...
    await db.query("DELETE FROM session_participants WHERE room_id = $1", [id])
...
const leaveClassroomSession = async (req, res) => {
  try {
    ...
    await db.query("DELETE FROM session_participants WHERE room_id = $1 AND student_id = $2", [roomId, userId])
...
const fetchParticipants = async (req, res) => {
  try {
    const { roomId } = req.params
    const result = await db.query("SELECT user_id, avatar_url FROM session_participants WHERE room_id = $1", [roomId])
